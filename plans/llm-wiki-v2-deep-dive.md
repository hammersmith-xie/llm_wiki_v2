...
| Graph view / chat graph | `src/lib/wiki-graph.ts`, `src/lib/typed-graph.ts`, `src/lib/graph-relevance.ts`, `src/components/chat/chat-panel.tsx` | Visual graph and chat context expansion now include explicit v2 typed relationship arrays, while Leiden clustering, layout, and relevance weighting still use the existing graph helpers. |
