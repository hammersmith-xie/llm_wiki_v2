      ...
      if (relativePath === "wiki/log.md" || relativePath.endsWith("/log.md")) {
        ...
        if (enriched.changed) {
          appendLifecycleAuditEvent(projectPath, {
            ...
            console.warn(
              `[lifecycle] audit write failed for ${relativePath}: ${err instanceof Error ? err.message : err}`,
            )
          })
        }
      }
...
export function buildGenerationPrompt(schema: string, purpose: string, index: string, sourceFileName: string, overview?: string, sourceContent: string = ""): string {
  ...
  return [
    ...
    "",
    "LLM Wiki v2 lifecycle fields (include them when you can; the app will fill sane defaults if omitted):",
    "  • lifecycle             — working | episodic | semantic | procedural | archived",
    "  • confidence            — decimal string between 0 and 1, e.g. `\"0.72\"`",
    "  • confidence_reasons    — short array explaining source support, recency, contradictions, or uncertainty",
    "  • last_confirmed        — date in YYYY-MM-DD form; use the source/ingest date if no better date exists",
    "  • reinforcement_count   — integer string, usually `\"0\"` for brand-new knowledge",
    "  • supersedes            — array of older page slugs this page replaces",
    "  • superseded_by         — array of newer page slugs that replace this page",
    "  • quality_score         — decimal string between 0 and 1",
    ...
    "",
    "Claim-friendly body rules:",
    "- Write high-value factual claims, findings, decisions, recommendations, contradictions, and conclusions as standalone bullets or short paragraphs.",
    "- Keep each durable claim precise enough that the app can score source support, recency, contradictions, and reinforcement later.",
    "- Claim anchors use HTML comments like `<!-- claim:claim_xxx -->`, but they are app-managed: do not generate claim ids or manually add claim anchors unless preserving an existing anchor.",
    "- `.llm-wiki/claims.jsonl` is a derived app-owned index for search evidence, Memory Ops patrol, rebuild, and audit handoff; it is not a source of truth.",
    "- Do not create claim-like prose for every sentence. Prefer only durable, review-worthy facts.",
    "",
