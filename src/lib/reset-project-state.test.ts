...
// Dynamic-import mocks: resetProjectState uses runtime imports. vi.mock hoists
// these so the promises resolve to our stubs immediately.
...
describe("resetProjectState — module-level caches are awaited", () => {
  ...
  it("calls clearTypedGraphCache before the returned promise resolves", async () => {
    await resetProjectState()
    expect(mockClearTypedGraphCache).toHaveBeenCalledOnce()
  })

  it("ordering: when resolve() fires, all module caches are already cleared", async () => {
    // This is the regression guard against fire-and-forget resets.
    // By the time the outer await returns, every clear must be done.
    await resetProjectState()
    expect(mockPauseQueue).toHaveBeenCalledOnce()
    expect(mockClearGraphCache).toHaveBeenCalledOnce()
    expect(mockClearTypedGraphCache).toHaveBeenCalledOnce()
  ...
  it("does not throw when clearTypedGraphCache itself throws", async () => {
    mockClearTypedGraphCache.mockImplementationOnce(() => {
      throw new Error("boom")
    })
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {})
