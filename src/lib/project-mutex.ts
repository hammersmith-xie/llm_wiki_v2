...
export async function withProjectLock<T>(
  ...
  try {
    ...
    if (locks.get(projectPath) === tail || locks.size > 1024) {
...
/** Test-only — expose lock table size to verify completed tails are cleaned up. */
export function __projectLockCountForTesting(): number {
  return locks.size
}
