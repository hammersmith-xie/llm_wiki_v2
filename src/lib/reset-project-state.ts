...
export async function resetProjectState(): Promise<void> {
  ...
  const [queueMod, dedupQueueMod, graphMod, typedGraphMod] = await Promise.allSettled([
    import("@/lib/ingest-queue"),
    import("@/lib/dedup-queue"),
    import("@/lib/graph-relevance"),
    import("@/lib/typed-graph"),
  ])
  ...
  if (typedGraphMod.status === "fulfilled") {
    try {
      typedGraphMod.value.clearTypedGraphCache()
    } catch (err) {
      console.warn("[Reset Project State] clearTypedGraphCache failed:", err)
    }
  } else {
    console.warn("[Reset Project State] Failed to load typed-graph:", typedGraphMod.reason)
  }
}
