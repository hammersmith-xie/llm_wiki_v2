...
describe("mergePageContent — LLM merge", () => {
  ...
  it("unions llm wiki v2 typed relationship arrays across re-ingests", async () => {
    const merger = vi.fn().mockResolvedValue(
      PAGE(
        [
          "type: concept",
          ...
          "created: 2026-04-09",
          "uses: [new-tool]",
          "depends_on: [new-prereq]",
          "contradicts: [old-claim]",
          "supports: [new-claim]",
          "supersedes: [older-page]",
          "superseded_by: [future-page]",
        ].join("\n"),
    ...
    const out = await mergePageContent(
      ...
      PAGE(
        [
          ...
          "created: 2026-04-09",
          "uses: [old-tool]",
          "depends_on: [old-prereq]",
          "contradicts: [prior-claim]",
          "supports: [prior-support]",
          "supersedes: [old-page]",
    ...
    expect(out).toMatch(/uses:\s*\[\s*"old-tool",\s*"new-tool"\s*\]/)
    expect(out).toMatch(/depends_on:\s*\[\s*"old-prereq",\s*"new-prereq"\s*\]/)
    expect(out).toMatch(/contradicts:\s*\[\s*"prior-claim",\s*"old-claim"\s*\]/)
    expect(out).toMatch(/supports:\s*\[\s*"prior-support",\s*"new-claim"\s*\]/)
