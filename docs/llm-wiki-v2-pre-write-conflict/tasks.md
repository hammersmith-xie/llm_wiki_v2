...
- 🐛 **遇到的问题**: 功能审核发现 `conflict.preview` 未实际写入 audit，且 `conflict.review` 缺少 review item id；任务总数也与最终审核轮数不一致。
- 🔧 **最终实现逻辑**: 补 `conflict.preview` 写入；`useReviewStore` 新增 `addItemsAndReturn` 返回新增/合并 review；ingest 将 `reviewItemId` 写入 `conflict.review`；报告见 `review-round-1.md`。
- 🎯 **关键决策**: 保持 review-store 原 `addItems` 行为不变，新增返回型 API 只供需要 audit 关联 id 的路径使用。
...
- 🐛 **遇到的问题**: `addItemsAndReturn` 类型正确但缺返回值语义测试；ingest catch 缩进影响可读性。
- 🔧 **最终实现逻辑**: 补 review-store 新增/合并返回 id 测试，整理缩进；报告见 `review-round-2.md`。
- 🎯 **关键决策**: 保持 `addItems` 原 API 不变，只为 audit 关联 id 增加返回型 API。
...
- 🐛 **遇到的问题**: audit 实现已经避免完整正文，但缺少针对 `claimText/pageExcerpt` 不入 audit 的直接测试。
- 🔧 **最终实现逻辑**: 补 conflict audit 泄漏断言；报告见 `review-round-4.md`。
- 🎯 **关键决策**: review/audit 只承载证据摘要和 reason，不复制候选正文或页面摘录。
